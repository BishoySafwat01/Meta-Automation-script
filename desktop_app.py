...
===============================================================================
Apple Prismatic Glass Desktop Hub via pywebview (V6.6-HYBRID)
Architecture:
...
class DesktopBridgeApi:
    ...
    def link_rule_by_code(
        ...
        Requires a non-empty concurrency token. Dispatches runtime snapshot on success."""
        ...
        res = self.pm.link_rule_by_code(
            profile_name, rule_id, existing_rule_code,
        ...
        if res.get("ok"):
            ...
            res.setdefault("disk_ok", False)
            res.setdefault("runtime_refresh_successes", [])
            res.setdefault("runtime_refresh_failures", {})
        return res
    ...
    def save_profile_config(
        ...
        In V6.6, tokenless saves are strictly prohibited. Callers must supply expected_sha."""
        warnings.warn(
            "save_profile_config is deprecated in V6.6; use save_profile_config_coordinated instead.",
            DeprecationWarning,
            stacklevel=2,
        )
...
def run_desktop_app(dev_tools: bool = False):
    ...
    window = webview.create_window(
        title="Meta Automation Hub - Hybrid Edition (V6.6)",
...
def main():
    parser = argparse.ArgumentParser(
        description="Meta Automation Hub - Hybrid Edition Desktop (V6.6)"
    )
