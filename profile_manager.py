...
# -*- coding: utf-8 -*-
"""
Meta Business Suite Profile Manager Module (V6.6 Hybrid Release)
...
VALID_MATCH_TYPES: Set[str] = {"ultra_exact", "contains", "exact", "word", "regex"}
...
def get_rule_allowlist_canonical(rule: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "matchType": str(rule.get("matchType", "contains")),
...
def normalize_local_rules(rules: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rules:
        ...
        if not rule_copy.get("matchType"):
            rule_copy["matchType"] = "contains"
        elif rule_copy.get("matchType") not in VALID_MATCH_TYPES:
            rule_copy["matchType"] = "contains"
...
class ProfileLease:
    ...
    def __init__(self, profile_dir: Path):
        ...
        self.lock_file_path = self.profile_dir / ".v66_worker.lock"
...
class ProfileRootOwnershipGuard:
    """Acquires and holds an OS advisory lock on .v66_app_owner.lock on the profile root
    ...
    def __init__(self, base_dir: Path):
        ...
        self.lock_path = self.base_dir / ".v66_app_owner.lock"
        self._fd: Optional[int] = None
...
class ProfileManager:
    ...
    def resolve_base_dir() -> Path:
        ...
        if sys.platform.startswith("win"):
            ...
            return Path(userprofile) / "MetaInboxBot_V66_Hybrid_Profiles"
        return Path.home() / ".config" / "meta_inbox_bot_v66_hybrid" / "profiles"
    ...
    def is_profile_locked(self, name: str) -> bool:
        """Check if a profile has active locks (SingletonLock or .v66_worker.lock)."""
        ...
        worker_lock = pdir / ".v66_worker.lock"
    ...
    def link_rule_by_code(
        ...
        """Link a local rule to an existing shared ruleCode by adopting its canonical payload.

        Transaction contract:
        - Requires non-empty expected_target_sha256 (fail-closed).
        - Validates profile name, rule_id format, and Crockford Base32 code.
        - Locates rule_id in target; rejects RULE_NOT_FOUND if absent.
        - Rejects DUPLICATE_RULE_CODE if existing_rule_code already in target.
        - Scans all other profiles; rejects RULE_CODE_NOT_FOUND if code absent everywhere.
        - Verifies canonical payload consensus among all participants; returns LINK_CONFLICT
          with zero writes if any two participants disagree.
        - Adopts canonical payload while preserving local id, name, active, position.
        - Stages target document; re-verifies SHA immediately before write.
        - Atomic write with exact raw-byte rollback on failure.
        """
        # 1. Require concurrency token
        if not expected_target_sha256:
            return {
                "ok": False,
                "disk_ok": False,
                "code": "MISSING_CONCURRENCY_TOKEN",
                "message": "رمز التزامن للبروفايل الهدف مطلوب لعملية الربط بالكود.",
                "sha256_token": None,
                "ruleCode": None,
                "linked_profile_count": 0,
                "participating_profiles": [],
                "modified_profiles": [],
                "runtime_refresh_successes": [],
                "runtime_refresh_failures": {},
            }

        # 2. Validate code format
        if not existing_rule_code or not RULE_CODE_REGEX.match(str(existing_rule_code)):
            return {
                "ok": False,
                "disk_ok": False,
                "code": "INVALID_RULE_CODE",
                "message": f"كود القاعدة '{existing_rule_code}' غير صالح. يجب أن يتطابق مع النمط: MBS-XXXXXXXX.",
                "sha256_token": None,
                "ruleCode": None,
                "linked_profile_count": 0,
                "participating_profiles": [],
        ...
        if not isinstance(rule_id, str) or not rule_id:
            return {
                ...
                "message": "معرّف القاعدة المحلية غير صالح.",
                "sha256_token": None,
        ...
        with self._write_mutex:
            if not self.ownership_guard.is_owned():
                if not self.ownership_guard.acquire():
                    return {
                        ...
                        "disk_ok": False,
                        "code": "OWNERSHIP_COLLISION",
                        "message": "لا يمكن ربط القاعدة: المجلد الرئيسي مقفل بواسطة جلسة تطبيق أخرى نشطة.",
                        "sha256_token": None,
                        "ruleCode": None,
                        "linked_profile_count": 0,
                        "participating_profiles": [],
            ...
            # 3. Load and verify target profile
            target_cfg_path = self.get_profile_config_path(safe_name)
            if not target_cfg_path.is_file():
                return {
                    "ok": False,
                    "disk_ok": False,
                    "code": "TARGET_NOT_FOUND",
                    "message": f"تعذر قراءة البروفايل الهدف '{safe_name}': الملف غير موجود.",
                    "sha256_token": None,
                    "ruleCode": None,
                    "linked_profile_count": 0,
                    "participating_profiles": [],
            ...
            if target_sha != expected_target_sha256:
                return {
                    ...
                    "disk_ok": False,
                    "code": "STALE_CONFIG",
                    "message": f"تم تعديل البروفايل الهدف '{safe_name}' على القرص منذ آخر تحميل.",
                    "sha256_token": target_sha,
                    "ruleCode": None,
                    "linked_profile_count": 0,
                    "participating_profiles": [],
                    "modified_profiles": [],
            ...
            try:
                ...
                return {
                    ...
                    "disk_ok": False,
                    "code": "TARGET_PARSE_ERROR",
                    "message": f"تعذر قراءة البروفايل الهدف '{safe_name}': {e}",
                    "sha256_token": None,
                    "ruleCode": None,
                    "linked_profile_count": 0,
                    "participating_profiles": [],
            ...
            target_rules = target_doc.get("rules", [])
            if not isinstance(target_rules, list):
                target_rules = []

            # 4. Locate rule_id in target
            target_rule = None
            ...
            if target_rule is None:
                return {
                    ...
                    "disk_ok": False,
                    "code": "RULE_NOT_FOUND",
                    "message": f"القاعدة بالمعرّف '{rule_id}' غير موجودة في البروفايل '{safe_name}'.",
                    "sha256_token": target_sha,
                    "ruleCode": None,
                    "linked_profile_count": 0,
                    "participating_profiles": [],
            ...
            # 5. Reject duplicate code inside target
            for r in target_rules:
                if isinstance(r, dict) and r.get("ruleCode") == existing_rule_code and r.get("id") != rule_id:
                    return {
                        "ok": False,
                        "disk_ok": False,
                        "code": "DUPLICATE_RULE_CODE",
                        "message": f"كود القاعدة المشترك '{existing_rule_code}' موجود بالفعل داخل البروفايل الهدف.",
                        "sha256_token": target_sha,
                        "ruleCode": existing_rule_code,
                        "linked_profile_count": 0,
                        "participating_profiles": [],
            ...
            # 6. Scan all other profiles for existing_rule_code — collect participants
            participants: List[Dict[str, Any]] = []  # [{profile_name, sha256_token, rule, doc}]
            for prof in self.list_profiles():
                pname = prof["name"]
                if pname == safe_name:
                    continue
                pres = self.load_profile_config_result(pname)
                if not pres.get("ok") or not isinstance(pres.get("data"), dict):
                    continue
                pdata = pres["data"]
                for r in pdata.get("rules", []):
                    if isinstance(r, dict) and r.get("ruleCode") == existing_rule_code:
                        participants.append({
                            "profile_name": pname,
                            "sha256_token": pres.get("sha256_token"),
                            "rule": r,
                            "doc": pdata,
                        })
                        break
            ...
            if not participants:
                return {
                    "ok": False,
                    "disk_ok": False,
                    "code": "RULE_CODE_NOT_FOUND",
                    "message": f"كود القاعدة '{existing_rule_code}' غير موجود في أي بروفايل آخر. يجب أن يكون الكود موجوداً في بروفايل آخر لإجراء الربط.",
                    "sha256_token": target_sha,
                    "ruleCode": existing_rule_code,
                    "linked_profile_count": 0,
                    "participating_profiles": [],
            ...
            # 7. Verify canonical payload consensus among all participants
            canonical_ref = get_rule_allowlist_canonical(participants[0]["rule"])
            conflict_participants = [
                {
                    "profile_name": p["profile_name"],
                    "sha256_token": p["sha256_token"],
                    "rule": {
                        "id": p["rule"].get("id"),
                        "name": p["rule"].get("name", ""),
                        "ruleCode": existing_rule_code,
                        "keywords": list(p["rule"].get("keywords", [])),
                        "keyword": p["rule"].get("keyword", ""),
                        "reply": str(p["rule"].get("reply", "")),
                        "matchType": str(p["rule"].get("matchType", "ultra_exact")),
                        "caseSensitive": bool(p["rule"].get("caseSensitive", False)),
                        "contextKeywords": list(p["rule"].get("contextKeywords", [])) if isinstance(p["rule"].get("contextKeywords"), list) else [],
                        "active": bool(p["rule"].get("active", True)),
                    },
                }
                for p in participants
            ]
            if any(get_rule_allowlist_canonical(p["rule"]) != canonical_ref for p in participants):
                return {
                    "ok": False,
                    "disk_ok": False,
                    "code": "LINK_CONFLICT",
                    "message": f"يوجد تضارب في محتوى القاعدة المشتركة '{existing_rule_code}' بين البروفايلات المشاركة.",
                    "conflicting_code": existing_rule_code,
                    "sha256_token": target_sha,
                    "ruleCode": existing_rule_code,
                    "linked_profile_count": len(participants),
                    "participating_profiles": conflict_participants,
                    "modified_profiles": [],
                    "runtime_refresh_successes": [],
                    "runtime_refresh_failures": {},
                }

            # 8. Adopt canonical payload — preserve local id, name, active, position
            canonical_rule = participants[0]["rule"]
            target_rule["ruleCode"] = existing_rule_code
            target_rule["keywords"] = list(canonical_rule.get("keywords", [])) if isinstance(canonical_rule.get("keywords"), list) else []
            target_rule["keyword"] = ", ".join(target_rule["keywords"])
            target_rule["reply"] = str(canonical_rule.get("reply", ""))
            target_rule["matchType"] = str(canonical_rule.get("matchType", "ultra_exact"))
            target_rule["caseSensitive"] = bool(canonical_rule.get("caseSensitive", False))
            target_rule["contextKeywords"] = list(canonical_rule.get("contextKeywords", [])) if isinstance(canonical_rule.get("contextKeywords"), list) else []
            target_rule["contextMatchType"] = str(canonical_rule.get("contextMatchType", "contains"))
            if target_rule["contextKeywords"]:
                target_rule["contextKeyword"] = ", ".join(target_rule["contextKeywords"])
            else:
                target_rule.pop("contextKeyword", None)
            # id, name, active, and position in list are deliberately untouched

            target_doc["rules"] = target_rules

            # 9. Backup and re-verify target and every canonical participant
            # immediately before write. A peer changing after canonical discovery
            # invalidates the payload we are about to join, even though only the
            # local target file is replaced by this operation.
            backup_bytes = target_cfg_path.read_bytes() if target_cfg_path.is_file() else b""
            recheck_sha = hashlib.sha256(backup_bytes).hexdigest() if backup_bytes else None
            if recheck_sha != expected_target_sha256:
                return {
                    "ok": False,
                    "disk_ok": False,
                    "code": "STALE_CONFIG",
                    "message": f"تم تعديل البروفايل الهدف '{safe_name}' على القرص أثناء إعداد عملية الربط.",
                    "sha256_token": recheck_sha,
                    "ruleCode": None,
                    "linked_profile_count": len(participants),
                    "participating_profiles": conflict_participants,
                    "modified_profiles": [],
            ...
            for participant in participants:
                participant_name = participant["profile_name"]
                participant_path = self.get_profile_config_path(participant_name)
                participant_bytes = participant_path.read_bytes() if participant_path.is_file() else b""
                participant_sha = hashlib.sha256(participant_bytes).hexdigest() if participant_bytes else None
                if participant_sha != participant["sha256_token"]:
                    return {
                        "ok": False,
                        "disk_ok": False,
                        "code": "STALE_CONFIG",
                        "message": f"تم تعديل البروفايل المرتبط '{participant_name}' أثناء إعداد عملية الربط.",
                        "stale_profile": participant_name,
                        "sha256_token": recheck_sha,
                        "current_sha256": participant_sha,
                        "ruleCode": existing_rule_code,
                        "linked_profile_count": len(participants),
                        "participating_profiles": conflict_participants,
                        "modified_profiles": [],
            ...
            # 10. Atomic write with exact raw-byte rollback on failure
            try:
                atomic_write_json(target_cfg_path, target_doc)
            except Exception as e:
                try:
            ...
            new_bytes = target_cfg_path.read_bytes()
            ...
            return {
                ...
                "disk_ok": True,
                "code": "OK",
                "message": f"تم ربط القاعدة بكود '{existing_rule_code}' بنجاح وتبنّي المحتوى الموحّد.",
                "sha256_token": new_sha,
                "ruleCode": existing_rule_code,
                "linked_profile_count": len(participants) + 1,
                "participating_profiles": conflict_participants,
                "modified_profiles": [safe_name],
                "runtime_refresh_successes": [],
                "runtime_refresh_failures": {},
            }
