...
(function () {
  ...
  const state = {
    ...
    recentSentMessages: new TTLMap(1000, 10 * 60 * 1000), // threadKey + exact sent text -> true
  ...
  function markProcessed(threadKey, contactKey = null, rowFingerprint = null) {
    if (threadKey) state.processedContacts.add(threadKey);
  ...
  function checkAndRehydrateTenant(hud) {
    ...
    if (latestTenantId !== state.currentTenantId) {
      ...
      if (state.recentSentMessages) state.recentSentMessages.clear();
  ...
  const DOM = {
    ...
    dismissComposerSuggestions(composerTarget = null) {
      try {
        ...
        const selectors = [
          'div[role="dialog"]',
          'div[aria-label*="اقتراح" i]',
          'div[aria-label*="suggest" i]',
          '[data-testid*="suggest" i]',
          '[aria-label*="رد مقترح" i]',
          '[aria-label*="suggested reply" i]',
          'button[aria-label*="إغلاق" i]',
          'button[aria-label*="Close" i]'
        ];
        const traps = Array.from(document.querySelectorAll(selectors.join(','))).filter(el => {
          if (el.closest('#mbs-inbox-automator-root') || el.offsetParent === null) return false;
          const rect = el.getBoundingClientRect();
          const overlapsHorizontally = rect.right >= composerRect.left - 60 && rect.left <= composerRect.right + 60;
          const isNearComposer = rect.bottom >= composerRect.top - 360 && rect.top <= composerRect.bottom + 180;
          return rect.width > 0 && rect.height > 0 && overlapsHorizontally && isNearComposer;
        });

        if (traps.length === 0) return false;
        for (const trap of traps) {
          const closeBtn = trap.matches?.('button[aria-label*="إغلاق" i], button[aria-label*="Close" i]')
            ? trap
            : trap.querySelector?.('button[aria-label*="إغلاق" i], button[aria-label*="Close" i], div[role="button"][aria-label*="إغلاق" i], div[role="button"][aria-label*="Close" i]');
          if (closeBtn) {
            try { dispatchFullClick(closeBtn); } catch (_) {}
          }
        }
        this.sendEscape();
        return true;
    ...
    forceClearComposer(composerTarget = null) {
      try {
        const composer = composerTarget || this.resolveComposer();
        if (!composer) return false;
        try {
        ...
        try { if ('value' in composer) composer.value = ''; } catch (_) {}
        try { composer.textContent = ''; } catch (_) {}
        try { composer.innerHTML = ''; } catch (_) {}
        try {
    ...
    registerSentMessage(threadKey, text) {
      const normalizedText = normalizeArabicText(text);
      if (!normalizedText || !state.recentSentMessages) return;
      const activeThreadKey = this.getActiveThreadKey();
      const keys = new Set([threadKey, activeThreadKey].filter(Boolean));
      for (const key of keys) {
        state.recentSentMessages.set(`${key}__${normalizedText}`, true);
      }
    },

    isRecentlySentMessage(threadKey, text) {
      if (!threadKey || !text || !state.recentSentMessages) return false;
      const normalizedText = normalizeArabicText(text);
      return Boolean(normalizedText && state.recentSentMessages.has(`${threadKey}__${normalizedText}`));
    },

    finalizeCompletedReply(composerTarget = null) {
      const composer = composerTarget || this.resolveComposer();
      try { this.dismissComposerSuggestions(composer); } catch (_) {}
      try { this.forceClearComposer(composer); } catch (_) {}
      try { this.sendEscape(); } catch (_) {}
      try { if (composer && typeof composer.blur === 'function') composer.blur(); } catch (_) {}
      try {
        const selection = window.getSelection();
        if (selection) selection.removeAllRanges();
      } catch (_) {}
      try { if (composer) botDraftOwnership.delete(composer); } catch (_) {}
      try { releaseChatFocus(); } catch (_) {}
      return true;
    },
    ...
    async waitForComposerClear(composer, maxWaitMs = 1500) {
      ...
      while (Date.now() - startTime < maxWaitMs) {
        if (this.dismissComposerSuggestions(composer)) {
          this.forceClearComposer(composer);
          return true;
        }
      ...
      // Emergency fallback if Lexical or a Meta suggestion retains text after dispatch
      this.dismissComposerSuggestions(composer);
      this.forceClearComposer(composer);
      return true;
    },
    ...
    isOutboundBubble(bubble) {
      ...
      // 0. Exact local send history always wins over visual/layout heuristics.
      const activeThreadKey = this.getActiveThreadKey();
      if (this.isRecentlySentMessage(activeThreadKey, text)) {
        return true;
      }
  ...
  const HumanSimulator = {
    ...
    async typeIntoComposer(composer, text, logger, options = {}) {
      ...
      DOM.dismissComposerSuggestions(composer);
      DOM.forceClearComposer(composer);
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        if (i % 8 === 0 && DOM.dismissComposerSuggestions(composer)) {
          composer.focus();
          DOM.assertComposerLease(lease);
        }
      ...
      await sleep(350);
      if (state.emergencyAbort || !state.isRunning) throw new Error('ABORT_SIGNAL');
      if (DOM.dismissComposerSuggestions(composer)) {
        composer.focus();
        DOM.assertComposerLease(lease);
      }
      ...
      DOM.registerSentMessage(lease.threadKey, text);

      if (options.finalBubble) {
        await sleep(120);
        const suggestionWasVisible = DOM.dismissComposerSuggestions(composer);
        const remainingFinalContent = (composer.innerText || composer.textContent || '').trim();
        const remainingIsOurPayload = normalizeArabicText(remainingFinalContent) === normalizeArabicText(text);
        if (!suggestionWasVisible && remainingIsOurPayload) {
          const sendBtn = DOM.getSendButton();
          if (sendBtn) {
            await this.naturalClick(sendBtn);
          }
        }
        DOM.finalizeCompletedReply(composer);
        return { sent: true, threadKey: lease.threadKey };
      }
      ...
      const remainingIsOurPayload = normalizeArabicText(currentContent) === normalizeArabicText(text);

      if (!suggestionWasVisible && !isPlaceholder && remainingIsOurPayload) {
        logger.log('TYPING', 'الضغط الاحتياطي على زر الإرسال...');
        const sendBtn = DOM.getSendButton();
        if (sendBtn) {
          await this.naturalClick(sendBtn);
        }
      } else if (!isPlaceholder && currentContent.length > 0) {
        DOM.forceClearComposer(composer);
      }
      ...
      try {
        ...
        class OperationBudget {
          ...
          extendForActiveReply(replyBudgetMs) {
            const safeReplyBudget = Math.max(0, Number(replyBudgetMs) || 0);
            const elapsed = Date.now() - this.startTime;
            this.hardMaxMs = Math.max(this.hardMaxMs, elapsed + safeReplyBudget);
            this.touch(safeReplyBudget);
          }
            ...
            if (matchResult) {
              ...
              if (!composer) {
                ...
                if (replies.length === 0) {
                  ...
                  const maxTypingSpeed = Math.max(1, Number(state.config.maxTypingSpeed) || Number(state.config.typingSpeed) || 65);
                  const maxCooldown = Math.max(0, Number(state.config.maxCooldown) || 2500);
                  const replyBudget = Math.max(30000, (totalChars * maxTypingSpeed) + (replies.length * maxCooldown) + 15000);
                  if (operationBudget) {
                    operationBudget.extendForActiveReply(replyBudget);
                  }
                  ...
                  for (let idx = 0; idx < replies.length; idx++) {
                    ...
                    try {
                      await HumanSimulator.typeIntoComposer(composer, replies[idx], this.hud, {
                        finalBubble: idx === replies.length - 1
                      });
                    } catch (typeErr) {
                  ...
                  const completedThreadKey = DOM.getActiveThreadKey() || contactKey;
                  DOM.finalizeCompletedReply(composer);
                  markProcessed(completedThreadKey, contactKey, rowFingerprint);
                  this.hud.log('INFO', `تم إرسال كافة الردود بنجاح للعميل ${contactName || contactKey} (${replies.length} فقاعة). تم تحرير المحرر والعودة لمسح القائمة.`);
                  return { skipLoop: false, replyCompleted: true };
                }
              }
            } else {
