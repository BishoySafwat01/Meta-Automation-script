/**
 * Meta Automation Desktop Control Center (V6.6-HYBRID)
...
(function () {
  ...
  function renderRulesUI() {
    ...
    rules.forEach((rule, idx) => {
      ...
      card.innerHTML = `
        <div class="rule-meta-bar">
          <div class="rule-meta-left" style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
            ...
            <span class="rule-title">قاعدة #${idx + 1}: <span class="rule-name-display">${escapeHtml(rule.name || 'بدون اسم')}</span>
              <button type="button" class="btn-name-edit-pencil" title="تعديل الاسم">${ICONS.pencil}</button>
              — (كود: <bdi dir="ltr">${escapeHtml(rule.ruleCode || 'MBS-XXXXXXXX')}</bdi>)</span>
            ${rule.ruleCode ? `
            ...
            ` : `
              <button type="button" class="btn-link-by-code" title="ربط هذه القاعدة بكود موجود">ربط بكود موجود</button>
            `}
          </div>
          <div class="rule-meta-right">
            <select class="glass-select rule-match-type" title="نوع المطابقة">
              ...
              <option value="exact" ${(rule.matchType === 'exact' || rule.matchType === 'word') ? 'selected' : ''}>تطابق كامل بعد التطبيع (Exact)</option>
        ...
        <div class="link-by-code-panel" hidden>
          <label style="font-size:11px;font-weight:600;color:#4f46e5;white-space:nowrap;">ربط بكود:</label>
          <input class="link-code-input" placeholder="MBS-XXXXXXXX" maxlength="12" dir="ltr" autocomplete="off" spellcheck="false">
          <button type="button" class="btn-link-code-confirm">تأكيد الربط</button>
          <button type="button" class="btn-link-code-cancel">إلغاء</button>
          <span class="link-code-status"></span>
        </div>
        ...
        <div class="rule-section">
          <div class="rule-section-header" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              ...
              <span class="rule-section-hint">كل شريحة تحفظ الكلمة أو العبارة بدقة (مع الفواصل والمسافات والأسطر)</span>
            </div>
      ...
      // -----------------------------------------------------------------------
      // Inline rule-name edit (pencil in header)
      // -----------------------------------------------------------------------
      const nameDisplay = card.querySelector('.rule-name-display');
      const btnNamePencil = card.querySelector('.btn-name-edit-pencil');
      if (nameDisplay && btnNamePencil) {
        function startNameEdit() {
          const currentDisplay = card.querySelector('.rule-name-display');
          if (!currentDisplay || card.querySelector('.inline-name-edit-input')) return;
          const prev = rule.name || '';
          const inp = document.createElement('input');
          inp.type = 'text';
          inp.className = 'inline-name-edit-input';
          inp.value = prev;
          inp.setAttribute('aria-label', 'اسم القاعدة');
          currentDisplay.replaceWith(inp);
          btnNamePencil.style.display = 'none';
          inp.focus();
          inp.select();
          ...
          function finish(value) {
            ...
            newDisplay.className = 'rule-name-display';
            newDisplay.textContent = rule.name || 'بدون اسم';
            newDisplay.addEventListener('click', startNameEdit);
            inp.replaceWith(newDisplay);
            btnNamePencil.style.display = '';
          }

          function commit() {
            finish(inp.value); // verbatim, no trim
          }
          function cancel() {
            finish(prev);
          }
        ...
        btnNamePencil.addEventListener('click', (e) => { e.stopPropagation(); startNameEdit(); });
      }
      ...
      /**
       * Render compact chip for one keyword entry.
       * Data contract: rule.keywords[kwIdx] is always the verbatim stored value.
       * Stored values are never trimmed, split, normalized, or delimiter-reconstructed.
       */
      function makeKeywordChip(kwIdx, isContext) {
        const arr = isContext ? rule.contextKeywords : rule.keywords;
        const container = isContext ? contextContainer : keywordsContainer;
        const raw = arr[kwIdx];

        const chip = document.createElement('div');
        chip.className = 'keyword-chip';

        // Visible preview: replace literal newlines/tabs with markers for display only
        const preview = (raw || '')
          .replace(/\t/g, '⇥')
          .replace(/\n/g, '↵');
        const placeholder = isContext ? 'سياق الإعلان...' : 'الكلمة أو العبارة...';

        chip.innerHTML = `
          <span class="chip-text" title="${escapeHtml(raw || '')}">${raw ? escapeHtml(preview) : `<em style="opacity:0.5;font-style:normal;">${escapeHtml(placeholder)}</em>`}</span>
          <button type="button" class="chip-edit-btn" title="تعديل">${ICONS.pencil}</button>
          <button type="button" class="chip-remove" title="حذف">×</button>
        `;

        function startChipEdit() {
          if (chip.classList.contains('editing')) return;
          const prevVal = arr[kwIdx];
          chip.classList.add('editing');
          chip.innerHTML = '';

          const ta = document.createElement('textarea');
          ta.className = 'chip-edit-textarea';
          ta.value = prevVal;
          ta.rows = Math.max(1, (prevVal.match(/\n/g) || []).length + 1);
          ta.setAttribute('aria-label', placeholder);
          chip.appendChild(ta);

          let settled = false;

          function finishChipEdit(value) {
            if (settled) return;
            settled = true;
            arr[kwIdx] = value;
            if (isContext) {
            ...
            renderChips(isContext);
          }

          function commitChipEdit() {
            finishChipEdit(ta.value); // verbatim — never trim
          }
          function cancelChipEdit() {
            finishChipEdit(prevVal);
          }
          ...
          ta.addEventListener('keydown', (e) => {
            ...
            // plain Enter inserts literal newline — no special handling needed (default textarea behavior)
          });
          ta.addEventListener('blur', () => {
            if (!settled) commitChipEdit();
          });
        ...
        chip.querySelector('.chip-text').addEventListener('click', startChipEdit);
      ...
      function renderChips(isContext) {
        const arr = isContext ? rule.contextKeywords : rule.keywords;
        const container = isContext ? contextContainer : keywordsContainer;
        if (!container) return;
        container.innerHTML = '';
        if (!isContext && (!Array.isArray(arr) || arr.length === 0)) {
          rule.keywords = [''];
        }
        const activeArr = isContext ? rule.contextKeywords : rule.keywords;
        activeArr.forEach((_, idx2) => {
          container.appendChild(makeKeywordChip(idx2, isContext));
        });
      }
      ...
      if (btnAddKeyword) {
        btnAddKeyword.addEventListener('click', () => {
          ...
          rule.keywords.push('');
          renderChips(false);
          // open the last chip editor automatically
          const chips = keywordsContainer.querySelectorAll('.keyword-chip');
          if (chips.length > 0) {
            chips[chips.length - 1].querySelector('.chip-text')?.click();
          }
        });
      }

      // -----------------------------------------------------------------------
      // Compact context-keyword chips
      // -----------------------------------------------------------------------
      ...
      if (btnAddContext) {
        btnAddContext.addEventListener('click', () => {
          ...
          renderChips(true);
          const chips = contextContainer.querySelectorAll('.keyword-chip');
          if (chips.length > 0) {
            chips[chips.length - 1].querySelector('.chip-text')?.click();
          }
        });
      ...
      // Link-by-code panel
      // -----------------------------------------------------------------------
      const btnLinkByCode = card.querySelector('.btn-link-by-code');
      const linkPanel = card.querySelector('.link-by-code-panel');
      if (btnLinkByCode && linkPanel) {
        const linkInput = linkPanel.querySelector('.link-code-input');
        const linkConfirm = linkPanel.querySelector('.btn-link-code-confirm');
        const linkCancel = linkPanel.querySelector('.btn-link-code-cancel');
        const linkStatus = linkPanel.querySelector('.link-code-status');
        const CROCKFORD_RE = /^MBS-[0-9A-HJKMNP-TV-Z]{8}$/;

        btnLinkByCode.addEventListener('click', (e) => {
          e.stopPropagation();
          linkPanel.hidden = !linkPanel.hidden;
          if (!linkPanel.hidden) {
            linkInput.value = '';
            linkStatus.textContent = '';
            linkStatus.className = 'link-code-status';
            linkInput.focus();
          }
        });
        ...
        linkConfirm.addEventListener('click', async () => {
          ...
          if (!CROCKFORD_RE.test(code)) {
            linkStatus.className = 'link-code-status';
            linkStatus.textContent = 'كود غير صالح. المطلوب: MBS- متبوعاً بـ 8 أحرف Crockford.';
            return;
          }
          linkStatus.textContent = 'جارٍ الربط...';
          linkStatus.className = 'link-code-status';
          try {
            const res = await callApi(
              ...
              rule.id,
              code,
              state.currentConfigSha256
            );
            if (res && res.ok) {
              if (res.sha256_token) state.currentConfigSha256 = res.sha256_token;
              linkStatus.textContent = 'تم الربط بنجاح!';
              linkStatus.className = 'link-code-status success';
              setTimeout(() => { linkPanel.hidden = true; }, 1200);
              await refreshProfiles();
              ...
              linkStatus.className = 'link-code-status';
              linkStatus.textContent = (res && res.message) ? res.message : 'فشل الربط.';
            }
          } catch (err) {
            linkStatus.className = 'link-code-status';
            linkStatus.textContent = 'خطأ في الاتصال: ' + (err && err.message ? err.message : err);
          }
        });

        linkInput.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') linkConfirm.click();
          else if (e.key === 'Escape') { linkPanel.hidden = true; }
        });
      }
      ...
      if (btnCopyCode) {
        btnCopyCode.addEventListener('click', async (e) => {
          ...
          if (navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
            try { await navigator.clipboard.writeText(codeVal); copied = true; } catch (_) {}
          }
          if (!copied) {
            try {
              const ta = document.createElement('textarea');
  ...
  function setupEventListeners() {
    ...
    elements.btnAddRule.addEventListener('click', async () => {
      ...
      const firstChip = firstCard?.querySelector('.keyword-chip .chip-text');
      if (firstChip) {
        setTimeout(() => firstChip.click(), 60);
      }
    });
    ...
    elements.btnSaveRules.addEventListener('click', async () => {
      ...
      if (cards.length > 0 && Array.isArray(state.currentConfig.rules)) {
        cards.forEach((card, idx) => {
          ...
          // rule.name is kept current by the inline name editor — no DOM scrape needed
          ...
          // rule.keywords and rule.contextKeywords are kept current by chip event listeners
          // — reading from the model object is correct and authoritative.
          // Ensure compat fields are up-to-date at save time:
          if (Array.isArray(rule.keywords)) {
            rule.keyword = rule.keywords.join(', ');
          }
          if (Array.isArray(rule.contextKeywords)) {
