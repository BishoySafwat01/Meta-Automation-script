...
"""
=============================================================================
Meta Business Suite Inbox Auto-Responder & Unread Restorer (V6.6 Hybrid Release)
...
=============================================================================
  أتمتة صندوق بريد Meta Business Suite & استعادة غير مقروء (V6.6 Hybrid)
...
async def inject_hud_and_rules(
    ...
    try:
        ...
        is_already_loaded = await page.evaluate("""() => {
            return Boolean(
                window.__MBS_AUTOMATOR_V660_LOADED__ ||
...
async def run_attach_mode(p: Playwright, port: int, settings: dict, config_path: Path, auto_start: bool):
    ...
    try:
        ...
        if sys.platform.startswith("win"):
            print(f'{Colors.BOLD}chrome.exe --remote-debugging-port={port} --user-data-dir="%USERPROFILE%\\ChromeDevProfileV66Hybrid"{Colors.END}\n')
        else:
...
async def main():
    parser = argparse.ArgumentParser(
        description="Meta Business Suite Inbox Automator & Unread Restorer (Pure Python Zero-Extension Runner V6.6 Hybrid)"
    )
    ...
    parser.add_argument("--port", type=int, default=9660, help="Chrome Remote Debugging Port for --attach (default: 9660; isolated from legacy releases)")
