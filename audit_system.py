    ...
    def suite_static_integrity(self):
        ...
        for cvf in core_version_files:
            ...
            if p.is_file():
                ...
                if "V6.5.4" not in txt and "6.5.4" not in txt:
                    self.log_fail("Version Synchronization", f"{cvf} missing V6.5.4 tag")
                    version_ok = False
            else:
        ...
        if "window.__MBS_AUTOMATOR_V654_LOADED__" in js_src:
            pass
        else:
            self.log_fail("Version Synchronization", "Missing window.__MBS_AUTOMATOR_V654_LOADED__ in bot_script.js")
            version_ok = False

        if version_ok:
            self.log_pass("Version Synchronization", f"V6.5.4 & runtime sentinels verified across {len(core_version_files)} files")
...
def main():
    ...
    print(f"{C_BOLD}{C_CYAN}  MBS INBOX AUTOMATOR - MASTER ENTERPRISE FORENSIC AUDIT (V6.5.4)           {C_RESET}")
    ...
    if engine.failed == 0:
        print(f"\n{C_BOLD}{C_GREEN}FINAL VERDICT: 100% AUDIT PASS - ENTERPRISE PRODUCTION READY (V6.5.4)!{C_RESET}\n")
        sys.exit(0)
    else:
