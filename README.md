# أتمتة صندوق بريد Meta Business Suite & استعادة غير مقروء (الإصدار المؤسسي V4.9.0)
### Meta Business Suite Inbox Auto-Responder & Unread Restorer (Pure Python Zero-Extension Edition)
> نظام أتمتة مؤسسي متكامل لإدارة وتدقيق صندوق رسائل **Meta Business Suite** (صفحات فيسبوك وإنستغرام) عبر لغة **Python** ومحرك **Playwright** بالحقن الأصلي المباشر (Zero-Extension) بدون الحاجة لتثبيت أي إضافات متصفح (مثل Tampermonkey)، مع عزل تخزين الصفحات المتعددة ومحرك إيقاف فوري (<10ms) وواجهة تحكم متطورة بتصميم Google Glass زجاجي مدمجة داخل صفحة فيسبوك مباشرة.  
> **المسؤول التقني:** Bishoy Safwat (Senior Automation Engineer)

---

## 🌟 الميزة الكبرى في V4.9.0: التشغيل النقي بدون إضافات (Zero-Extension Architecture)
في هذا الإصدار، تم إلغاء الاعتماد تماماً على إضافة Tampermonkey:
- **حقن أصلي ومستمر (Native Injection)**: يقوم محرك Playwright بحقن كود المحرك `bot_script.js` وواجهة التحكم التفاعلية فورياً في المتصفح وعبر `context.add_init_script` ليظل نشطاً مع كل تحديث أو تنقل بين الصفحات.
- **3 أنماط تشغيل مرنة**:
  1. **الربط بالمتصفح النشط (`--attach` / `-a`)**: الاتصال الفوري بجلسة Chrome المفتوحة لديك عبر منفذ CDP 9222 مع الحفاظ على تسجيل دخولك.
  2. **بروفايل معزول ومستقل (`--profile` / `-p [Name]`)**: فتح متصفح Chrome في مجلد بروفايل مخصص ودائم.
  3. **تشغيل متزامن لكافة الصفحات (`--all`)**: تشغيل `Profile_PageA` و `Profile_PageB` بالتزامن في عمليات معزولة دون تداخل (Zero Cross-Talk).
...
- **وضع الشبح (Ghost Mode 👻)**: يتحول إلى كبسولة مراقبة مصغرة (`110x32px`) خفيفة تعرض نبض الحالة وعدد الردود المنفذة.
